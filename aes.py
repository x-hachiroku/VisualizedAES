...
class AES:
    def __init__(self, master_key, iv, rounds, no_sub_bytes, no_shift_rows, no_mix_columns):
        self.rounds = rounds
        self._key_matrices = self._expand_key(master_key)
        self.iv = iv
        self.no_sub_bytes = no_sub_bytes
        self.no_shift_rows = no_shift_rows
        self.no_mix_columns = no_mix_columns

    def sub_bytes(self, s):
        if self.no_sub_bytes:
            return
        for i in range(4):
            for j in range(4):
                s[i][j] = s_box[s[i][j]]
    ...
    def decrypt_ecb(self, ciphertext):
        ...
        return b''.join([self.decrypt_block(block) for block in blocks])
...
if __name__ == '__main__':
    key = b'\x00' * 16
    iv = b'\x00' * 16
    aes = AES(key, iv, 10, False, False, False)

    img = cv2.imread('example.png', cv2.IMREAD_UNCHANGED)
    plaintext = img.tobytes()

    ciphertext = aes.encrypt_ecb(plaintext)
    encrypted_img = np.frombuffer(ciphertext, dtype=np.uint8).reshape(img.shape)
    cv2.imshow('Encrypted Image', encrypted_img)

    decrypted_img = aes.decrypt_ecb(ciphertext)
    assert plaintext == decrypted_img
